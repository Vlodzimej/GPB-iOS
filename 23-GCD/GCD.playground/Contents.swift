...
// Ответ: Для того, что бы измежать блокировки в 62-ой строке достаточно перевести sync -> async. При этом serialQueue.sync не будет вызывать блокировку, так как будет запущен из асинхронного фонового потока.
...
//Ответ: В строке 93 concurrentQueue.async заменить на concurrentQueue.sync
