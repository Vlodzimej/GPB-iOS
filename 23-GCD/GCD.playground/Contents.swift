...
// Ответ: Вывод цифр 1 и 9 выполняется на главной очереди(main). Последующие цифры выводятся асинхронно в отдельной последовательной очереди(serial Queue). В данной реализации порядок вывода цифры после 1 будет непредсказуемый, так как отсуствует синхронизация между main и seriaQueue.
...
// Ответ: Для того, что бы измежать блокировки в 61-ой строке достаточно перевести sync -> async. При этом serialQueue.sync не будет вызывать блокировку, так как будет запущен из асинхронного фонового потока.
...
//Ответ: В строке 94 concurrentQueue.async заменить на concurrentQueue.sync
